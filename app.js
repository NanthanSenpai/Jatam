...
let _dictFilter = 'all', _searchIdx = null, _detailSpeak = '', _lpTimer = null, _lpWord = '';
...
function switchPage(p) {
  ...
  setTimeout(()=>{ const pg=document.getElementById('pg-'+p); if(pg)bindLP(pg); },100);
}
...
// ── DICTIONARY ───────────────────────────────────────────────────
// JMdict data (loaded async)
let _jmdict = null;
async function loadJMdict(){
  if(_jmdict) return _jmdict;
  try{
    const r=await fetch('jmdict.json');
    const d=await r.json();
    _jmdict=d.entries||[];
    _searchIdx=null; // rebuild index with jmdict
  }catch(e){_jmdict=[];}
  return _jmdict;
}
...
// ── LONG PRESS (word lookup on any JP text) ──────────────────────
function getJPWord(el,x,y){
  const range=document.caretRangeFromPoint?document.caretRangeFromPoint(x,y):null;
  if(range){
    const txt=range.startContainer.textContent||'';const off=range.startOffset;
    const re=/[\u3040-\u30FF\u4E00-\u9FFF]+/g;let m;
    while((m=re.exec(txt))!==null){if(m.index<=off&&off<=m.index+m[0].length)return m[0];}
    re.lastIndex=0;
    const near=txt.substring(Math.max(0,off-3),Math.min(txt.length,off+5));
    const m2=near.match(/[\u3040-\u30FF\u4E00-\u9FFF]+/);if(m2)return m2[0];
  }
  return(el.textContent||'').match(/[\u3040-\u30FF\u4E00-\u9FFF]+/)?.[0]||null;
}
function showCtxPop(word,x,y){
  _lpWord=word;
  const pop=document.getElementById('ctxPop'),bg=document.getElementById('ctxBg');if(!pop)return;
  document.getElementById('ctxJp').textContent=word;
  document.getElementById('ctxRom').textContent='...';
  document.getElementById('ctxTa').textContent='';
  document.getElementById('ctxBody').innerHTML='<div style="padding:18px;text-align:center;font-family:var(--FT);font-size:12px;color:var(--tx3);">தகவல் தேடுகிறோம்...</div>';
  const vw=window.innerWidth,pw=Math.min(340,vw*.94);
  let left=x-pw/2,top=y+16;
  left=Math.max(8,Math.min(left,vw-pw-8));top=Math.max(70,Math.min(top,window.innerHeight-440));
  pop.style.left=left+'px';pop.style.top=top+'px';pop.style.width=pw+'px';
  pop.classList.add('on');if(bg)bg.style.display='block';
  speak(word);
  const isK=/[\u4E00-\u9FFF]/.test(word);isK?loadKanjiCtx(word):loadWordCtx(word);
}
function closeCtxPop(){document.getElementById('ctxPop')?.classList.remove('on');const bg=document.getElementById('ctxBg');if(bg)bg.style.display='none';_lpWord='';}
function speakCtx(){if(_lpWord)speak(_lpWord);}
function learnCtxWord(){if(_lpWord)markLearned(_lpWord);}
function loadWordCtx(word){
  const rom=document.getElementById('ctxRom'),ta=document.getElementById('ctxTa'),body=document.getElementById('ctxBody');
  const idx=buildSearchIndex();
  const found=idx.find(i=>i.jp===word||(i.type==='word'&&i.jp?.includes(word)));
  if(found&&found.type==='word'){
    rom.textContent=(found.kana||found.rom||'');ta.textContent=found.ta||'';
    body.innerHTML=buildCtxWordHTML(found);
  } else {
    rom.textContent='உள்ளூர் தரவில் இல்லை';
    body.innerHTML=`<div style="padding:14px;font-family:var(--FT);font-size:12px;color:var(--tx2);">
      <span style="color:var(--red);cursor:pointer;font-weight:700;" onclick="document.getElementById('dict-search').value='${word}';openDict();closeCtxPop();">📖 அகராதியில் தேடு →</span>
    </div>`;
  }
}
function loadKanjiCtx(word){
  const rom=document.getElementById('ctxRom'),ta=document.getElementById('ctxTa'),body=document.getElementById('ctxBody');
  const kj=KANJI.find(k=>k[0]===word);
  if(kj){
    rom.textContent=kj[1]+(kj[2]?' · '+kj[2]:'');
    ta.textContent=kj[4]+' ('+kj[3]+')';
    body.innerHTML=buildCtxKanjiHTML(kj);
  } else {body.innerHTML=`<div style="padding:14px;font-family:var(--FT);font-size:12px;">அகராதியில் இல்லை. <span style="color:var(--red);cursor:pointer;" onclick="openDict();closeCtxPop();">📖 தேடு →</span></div>`;}
}
function buildCtxWordHTML(w){
  return `<div class="ctx-grid">
    ${w.jlpt?`<div class="ctx-pill"><span class="lbl">JLPT</span><span class="bdg ${w.jlpt.toLowerCase()} val">${w.jlpt}</span></div>`:''}
    ${w.wtype?`<div class="ctx-pill"><span class="lbl">வகை</span><span class="val">${w.wtype}</span></div>`:''}
  </div>
  ${w.ta?`<div class="ctx-sec"><div class="ctx-sec-t">தமிழ் பொருள்</div><div class="ctx-pill" style="grid-column:1/-1"><span class="val ta">${w.ta}</span></div></div>`:''}
  ${w.exjp?`<div class="ctx-sec"><div class="ctx-sec-t">உதாரணம்</div>
    <div class="ctx-ex" onclick="speak('${(w.exjp||'').replace(/'/g,"\\'")}')">
      <div class="ctx-ex-jp">🔊 ${w.exjp}</div><div class="ctx-ex-ta">${w.exta||''}</div>
    </div></div>`:''}`;
}
function buildCtxKanjiHTML(k){
  return `<div class="ctx-grid">
    <div class="ctx-pill"><span class="lbl">音読み ON</span><span class="val jp">${k[1]||'—'}</span></div>
    <div class="ctx-pill"><span class="lbl">訓読み KUN</span><span class="val jp">${k[2]||'—'}</span></div>
    <div class="ctx-pill"><span class="lbl">JLPT</span><span class="bdg ${(k[5]||'').toLowerCase()} val">${k[5]}</span></div>
    <div class="ctx-pill"><span class="lbl">கோடுகள்</span><span class="val">${k[6]}</span></div>
  </div>
  ${k[4]?`<div class="ctx-sec"><div class="ctx-sec-t">தமிழ்</div><div class="ctx-pill"><span class="val ta">${k[4]}</span></div></div>`:''}
  ${k[7]?.length?`<div class="ctx-sec"><div class="ctx-sec-t">சொல் உதாரணங்கள்</div>
    ${k[7].map(e=>`<div class="ctx-ex" onclick="speak('${e[0].replace(/'/g,"\\'")}')">
      <div class="ctx-ex-jp" style="font-family:var(--FJ);font-size:15px;font-weight:700;">🔊 ${e[0]} <span style="font-family:var(--FM);font-size:8px;opacity:.6;">${e[1]}</span></div>
      <div class="ctx-ex-ta">${e[2]}</div></div>`).join('')}</div>`:''}`;
}
function bindLP(el){
  el.addEventListener('touchstart',e=>{
    const t=e.touches[0];const lx=t.clientX,ly=t.clientY;
    _lpTimer=setTimeout(()=>{
      const word=getJPWord(e.target,lx,ly);if(word)showCtxPop(word,lx,ly);
    },700);
  },{passive:true});
  ...
  el.addEventListener('contextmenu',e=>{
    const word=getJPWord(e.target,e.clientX,e.clientY);
    if(word){e.preventDefault();showCtxPop(word,e.clientX,e.clientY);}
  });
}
document.querySelectorAll('.pg').forEach(bindLP);
